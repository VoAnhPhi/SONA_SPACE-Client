...
export const getAllRooms = async (): Promise<Room[]> => {
      try {
            console.log(`Calling API: GET ${API_URL}/rooms`);
            ...
            if (!Array.isArray(response.data)) {
                  console.warn('API did not return an array for rooms, attempting to extract data');

                  // Thử lấy dữ liệu từ các cấu trúc phổ biến
                  const data = response.data.data || response.data.rooms || response.data.items || [];
            ...
            if (axios.isAxiosError(error)) {
                  ...
                  if (error.response?.status === 404) {
                        try {
                              ...
                              const altResponse = await axios.get(`${API_URL}/rooms`);
